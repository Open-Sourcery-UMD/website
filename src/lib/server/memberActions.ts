...
/**
 * Stores the current member count on the project for display. Best-effort:
 * the membership change already happened, and a stale count isn't worth
...
async function refreshTeamSize(project: ProjectRecord): Promise<void> {
  try {
    ...
    await adminDb()
      .collection("projects")
      .doc(project.id)
      .update({ currentTeamSize: collaborators.length + pendingInvitees.length });
  } catch (error) {
...
export async function joinProject(uid: string, projectId: unknown): Promise<void> {
  ...
  await withJoinLock(uid, async () => {
    // Joining your own proposed project (once its repo exists) is fine
    const pendingProposal = await findPendingProposal(uid, project.id);
    if (pendingProposal) {
      throw new HttpError(
        409,
    ...
    if (members.collaborators.length + members.pendingInvitees.length >= project.maxTeamSize) {
  ...
  await notifyLeadOfJoin(project, profile, login);
}
...
/**
 * Lets a project's lead remove another member from its repository
 */
export async function removeMember(
  ...
  if (!project.pointOfContact || project.pointOfContact !== uid) {
    throw new HttpError(403, "Only the project lead can remove members.");
  }
  ...
  // The token's own account manages the repository; removing it would cut
  // the site off from the project
  const botLogin = await getAuthenticatedUser();
  if (botLogin && normalizeLogin(botLogin) === normalizeLogin(target)) {
    throw new HttpError(400, "That account manages the repository and can't be removed.");
  }

  await removeUserFromRepository(target, GITHUB_ORG, project.repositoryName);
  await refreshTeamSize(project);
}
